...
void OnCreateFreeText(MainWindow* win, int x, int y) {
    ...
    auto annot = EngineMupdfCreateAnnotation(engine, AnnotationType::FreeText, pageNoUnderCursor, ptOnPage, 1);
    if (annot) {
...
void OnWindowContextMenu(MainWindow* win, int x, int y) {
    ...
    // modified on 20230616
    float zoompage = dm->GetZoomReal(pageNoUnderCursor);
    ...
    if (!IsClipboardFormatAvailable(CF_BITMAP)) {
        MenuRemove(popup, CmdCreateAnnotImage);
    }
    ...
    switch (cmd) {
        ...
        case CmdCreateAnnotCircle: {          
            auto annot = EngineMupdfCreateAnnotation(engine, annotType, pageNoUnderCursor, ptOnPage, zoompage);
            if (annot) {
                MainWindowRerender(win);
                ToolbarUpdateStateForWindow(win, true);
                createdAnnots.Append(annot);
        ...
        case CmdCreateAnnotRedact:
            createdAnnots = MakeAnnotationFromSelection(tab, AnnotationType::Redact);
            break;
        case CmdCreateAnnotBBox:
            createdAnnots = MakeAnnotationFromSelection(tab, AnnotationType::BBox);
            break;
