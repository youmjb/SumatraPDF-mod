...
Annotation* EngineMupdfCreateAnnotation(EngineBase* engine, AnnotationType typ, int pageNo, PointF pos, float zoom) {    
    ...
    if (typ == AnnotationType::Image) {
        ...
        // We couldn't have gotten this far if we couldn't get the hBitmap from the clipboard
        // Retrieve the bitmap handle from the clipboard.
        HBITMAP hBitmap = static_cast<HBITMAP>(GetClipboardData(CF_BITMAP));
        BITMAP bm;
        GetObject(hBitmap, sizeof(BITMAP), &bm);
        CloseClipboard();
        // Attaches hBitmap to the image object
        fz_image* image = render_to_pixmap(ctx, hBitmap, Size(bm.bmWidth, bm.bmHeight));
        if (image != nullptr)
        {
            pdf_set_annot_stamp_image(ctx, annot, image, zoom);
