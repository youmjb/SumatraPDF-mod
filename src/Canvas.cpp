...
static void Resizing(MainWindow* win, Point pt1, Point pt2) {        
    ...
    if (pageNo == PageNo(annot)) {
        ...
        {
            RectF arl = GetBounds(annot);     // get annotation position when left mouse button was clicked
            Rect rls{x2, y2, 1, 1};           // get mouse position
            RectF rl = dm->CvtFromScreen(rls, pageNo);    // convert mouse position into annotation position         
            if (ResizeMode == curTL) {
                ar.x = rl.x;
                ar.dx = arl.dx + arl.x - rl.x;
                ar.y = rl.y;
                ar.dy = arl.dy + arl.y - rl.y;
            } else if (ResizeMode == curBL) {
                ar.x = rl.x;
                ar.dx = arl.dx + arl.x - rl.x;
                ar.y = arl.y;
                ar.dy = arl.dy + rl.y - arl.y;                
            } else if (ResizeMode == curTR) {
                ar.x = arl.x;
                ar.dx = arl.dx + rl.x - arl.x;
                ar.y = rl.y;
                ar.dy = arl.dy + arl.y - rl.y;        
            } else if (ResizeMode == curBR) {
                ar.x = arl.x;
                ar.dx = arl.dx + rl.x - arl.x;
                ar.y = arl.y;
                ar.dy = arl.dy + rl.y - arl.y;   
...
static bool StopDraggingAnnotation(MainWindow* win, int x, int y, bool aborted) {
    ...
    if (pageNo == PageNo(annot)) {
        if (ResizeMode == curNone)
        {
            ...
            if (Type(annot) == AnnotationType::Line)
            {
                MoveLine(annot, r.x - ar.x, r.y - ar.y);
            }        
            else
            {
        ...
        if (Type(annot) == AnnotationType::Line) {
            Point pt1 = {0, 0};
            Point pt2 = {0, 0};
            Rect rScreenl{pt.x, pt.y, 1, 1};
            RectF rl = dm->CvtFromScreen(rScreenl, pageNo);            
            RectF rline = GetBounds(annot);
            if (ResizeMode == curTL)
            {
                pt1 = {int(rl.x), int(rl.y)};
                pt2 = {int(rline.x + rline.dx), int(rline.y + rline.dy)};
                ChangeLine(annot, pt1, pt2);
            } else if (ResizeMode == curBL) {
                pt1 = {int(rl.x), int(rl.y)};
                pt2 = {int(rline.x + rline.dx), int(rline.y)};
                ChangeLine(annot, pt1, pt2);
            } else if (ResizeMode == curTR) {
                pt1 = {int(rl.x), int(rl.y)};
                pt2 = {int(rline.x), int(rline.y + rline.dy)};
                ChangeLine(annot, pt2, pt1);
            } else if (ResizeMode == curBR) {
                pt1 = {int(rl.x), int(rl.y)};
                pt2 = {int(rline.x), int(rline.y)};
                ChangeLine(annot, pt2, pt1);
            }               
        }
