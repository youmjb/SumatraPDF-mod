...
static void Resizing(MainWindow* win, Point pt1, Point pt2) {        
    Annotation* annot = win->annotationOnLastButtonDown;   
    DisplayModel* dm = win->AsFixed();
    int pageNo = dm->GetPageNoByPoint(pt1);
    auto [x1, y1] = pt1;
    auto [x2, y2] = pt2;
    x1 += win->annotationBeingMovedOffset.x;
    y1 += win->annotationBeingMovedOffset.y;
    x2 += win->annotationBeingMovedOffset.x;
    y2 += win->annotationBeingMovedOffset.y;
    // we can only move annotation within the same page
    if (pageNo == PageNo(annot)) {
        // get new position of annotation (input: old position, gap between pt1 and pt2)
        RectF ar = GetBounds(annot);
        Rect r1Screen{x1, y1, 1, 1};
        RectF r1 = dm->CvtFromScreen(r1Screen, pageNo);
        Rect r2Screen{x2, y2, 1, 1};
        RectF r2 = dm->CvtFromScreen(r2Screen, pageNo);
        switch (ResizeMode) {
            case curTL:
                ar.x = r2.x;
                ar.dx += -r2.x + r1.x;
        ...
        if (Type(annot) == AnnotationType::FreeText) {
            SetRectHalf(annot, ar);     // save annotation position for only free text        
            WindowTab* tab = win->CurrentTab();
            auto w = win->CurrentTab()->editAnnotsWindow;
            SetMaxWidth(w, annot, ar, false);            		
        }        
        // save annotation position for every mouse movement except for free text and line annotation
        if (Type(annot) != AnnotationType::FreeText && Type(annot) != AnnotationType::Line)
        {
            SetRect(annot, ar);
        }       
        HWND hwnd = win->hwndCanvas;
        HDC hdc = GetDC(hwnd);
        // get screen point of new annotation
        Rect rectar = dm->CvtToScreen(pageNo, ar);
        // convert Rect to RECT
        RECT RECTar = {rectar.x, rectar.y, rectar.x + rectar.dx, rectar.y + rectar.dy};
        ...
        if (Type(annot) == AnnotationType::Line) {           
            ...
            Point ptla = dm->CvtToScreen(pageNo, la);
            Point ptlb = dm->CvtToScreen(pageNo, lb);          
            if (ResizeMode == curLa) {
                ::MoveToEx(hdc, ptlb.x, ptlb.y, NULL);
                ::LineTo(hdc, pt2.x, pt2.y);
            }            
            else if (ResizeMode == curLb) {
                ::MoveToEx(hdc, ptla.x, ptla.y, NULL);
                ::LineTo(hdc, pt2.x, pt2.y);
            }                
        }
        else {
            ::DrawEdge(hdc, &RECTar, EDGE_BUMP, BF_RECT);        
        }        
        ReleaseDC(hwnd, hdc);        
    }       
}
...
static void SetTextOrArrorCursor(DisplayModel* dm, Point pt) {
    if (dm->IsOverText(pt) || ResizeMode>=curTextCursor) {
        SetCursorCached(IDC_IBEAM);
    } else {
        SetCursorCached(IDC_ARROW);
    }
...
static void OnTimer(MainWindow* win, HWND hwnd, WPARAM timerId) {
    ...
    switch (timerId) {
        ...
        case REFRESH_TIMER_ID:
            if (GetCursor()==NULL && ResizeMode >= curTextCursor){
                SetCursorCached(IDC_IBEAM);
                ResizeMode--;
                if (ResizeMode < curTextCursor) ResizeMode = curNone;
            }
            else
                SetCursor(NULL);
            break;
