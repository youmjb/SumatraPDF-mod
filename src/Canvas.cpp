...
RECT sRect, eRect;
enum {
    curNone = 0,    // for move object
    curTL = 1,      // for rect, TL~B
    curTR = 2,
    curBL = 3,
    curBR = 4,
    curL = 5,
    ...
    curT = 7,
    curB = 8,
    curLa = 9,      // for line, a or b
    curLb = 10,
    curTextCursor = 11,   // for cursor input
};
...
static void Resizing(MainWindow* win, Point pt) {        
    ...
    if (pageNo == PageNo(annot)) {        
        ...
        switch (ResizeMode) {
            case curTL:
                eRect = {pt.x, pt.y, sRect.right, sRect.bottom};
                break;
            case curTR:
                eRect = {sRect.left, pt.y, pt.x, sRect.bottom};
                break;
            case curBL:
                eRect = {pt.x, sRect.top, sRect.right, pt.y};
                break;
            case curBR:
                eRect = {sRect.left, sRect.top, pt.x, pt.y};
                break;
            case curL:
                eRect = {pt.x, sRect.top, sRect.right, sRect.bottom};
                break;
            case curR:
                eRect = {sRect.left, sRect.top, pt.x, sRect.bottom};
                break;
            case curT:
                eRect = {sRect.left, pt.y, sRect.right, sRect.bottom};
                break;
            case curB:
                eRect = {sRect.left, sRect.top, sRect.right, pt.y};
                break;
            case curNone:
                break;
        }    
        ...
        if (Type(annot) == AnnotationType::Line) {           
            PointF la, lb;
            // get line point from previous line annotation before mouse dragging
            GetLinePoint(annot, &la, &lb);
            // convert line point to screen point coordinate
            Point ptcur = dm->CvtToScreen(pageNo, lb);
            if (ResizeMode == curLb)
                ptcur = dm->CvtToScreen(pageNo, la);
...
static bool StopDraggingAnnotation(MainWindow* win, int x, int y, bool aborted) {
    ...
    if (pageNo == PageNo(annot)) {        
        ...
        {
            Rect rScreenV = {eRect.left, eRect.top, eRect.right - eRect.left, eRect.bottom - eRect.top};
            RectF rv = dm->CvtFromScreen(rScreenV, pageNo);            
            SetRect(annot, rv);            
            ...
            if (Type(annot) == AnnotationType::Line)
            {
                ...
                PointF pflb = {};
                // get line point from previous line annotation before mouse dragging
                GetLinePoint(annot, &pfla, &pflb);
                // convert screen point coordinate to annotation point
                PointF ptl = dm->CvtFromScreen(pt, pageNo);
                if (ResizeMode == curLa) {
                    ChangeLine(annot, ptl, pflb);
                } else
                {
                    ChangeLine(annot, pfla, ptl);
                }                
            }
...
static void IsResizing(MainWindow* win, int x, int y) {
    DisplayModel* dm = win->AsFixed();
    Point pt{x, y};
    ...
    if (annot)
    {
        ...
        int iGap = 10;
        // set priority to resize object cursor, IDC_SIZEALL is for moving object cursor
        if (Type(annot) == AnnotationType::Line)
        {
            PointF pla = {};
            PointF plb = {};
            // get line point from previous line annotation before mouse dragging
            GetLinePoint(annot, &pla, &plb);
            // convert line point to screen point coordinate
            Point ptla = dm->CvtToScreen(pageNo, pla);
            Point ptlb = dm->CvtToScreen(pageNo, plb);
            if (abs(pt.x-ptla.x)<iGap && abs(pt.y-ptla.y)<iGap)
                SetCursorCached(IDC_HAND);
            else if (abs(pt.x-ptlb.x)<iGap && abs(pt.y-ptlb.y)<iGap)
                SetCursorCached(IDC_HAND);
            else
        ...
        {            
            if (abs(pt.x - rScreen.x) < iGap && abs(pt.y - rScreen.y) < iGap) {
                SetCursorCached(IDC_SIZENWSE);
            }
            else if (abs(pt.x - rScreen.x-rScreen.dx) < iGap && abs(pt.y - rScreen.y) < iGap) {
                SetCursorCached(IDC_SIZENESW);
            }
            else if (abs(pt.x - rScreen.x) < iGap && abs(pt.y - rScreen.y-rScreen.dy) < iGap) {
                SetCursorCached(IDC_SIZENESW);
            }
            else if (abs(pt.x - rScreen.x-rScreen.dx) < iGap && abs(pt.y - rScreen.y-rScreen.dy) < iGap) {
                SetCursorCached(IDC_SIZENWSE);
            }
            else if (abs(pt.x - rScreen.x) < iGap) {
                SetCursorCached(IDC_SIZEWE);
            }
            else if (abs(pt.x - rScreen.x-rScreen.dx) < iGap) {
                SetCursorCached(IDC_SIZEWE);
            }
            else if (abs(pt.y - rScreen.y) < iGap) {
                SetCursorCached(IDC_SIZENS);
            }
            else if (abs(pt.y - rScreen.y-rScreen.dy) < iGap) {
                SetCursorCached(IDC_SIZENS);
            }
            else {
                SetCursorCached(IDC_SIZEALL);
        ...
        // guide rectangle for resizing or moving of object
        HWND hwnd = win->hwndCanvas;
        HDC hdc = GetDC(hwnd);
        RECT RECTar = {rScreen.x, rScreen.y, rScreen.x+rScreen.dx, rScreen.y+rScreen.dy};
        ...
        lb.lbHatch = 0;
        hPen = ExtCreatePen(PS_GEOMETRIC | dwPenStyle[2], 4, &lb, 0, NULL);
        ...
        ::Rectangle(hdc, rScreen.x, rScreen.y, rScreen.x+rScreen.dx, rScreen.y+rScreen.dy);        
    ...
    else   // Refresh the page if the cursor is outside the annotation.
    {
        HWND hwnd = win->hwndCanvas;
        HDC hdc = GetDC(hwnd);
        InvalidateRect(hwnd, nullptr, false);
...
static void OnMouseMove(MainWindow* win, int x, int y, WPARAM) {
    ...
    switch (win->mouseAction) {
        ...
        case MouseAction::Idle:
            bool stTimer;
            transferFlagStopTimer(&stTimer);
            if (stTimer == true)
            {
                KillTimer(win->hwndCanvas, REFRESH_TIMER_ID);
                IsResizing(win, x, y);
                ResizeMode = curNone;
            }            
            break;
...
static void SetObjectUnderMouse(MainWindow* win, int x, int y) {
    ...
    if (annot) {
        ...
        // retangle information of annot
        RectF r = GetBounds(annot);
        Rect rScreen = dm->CvtToScreen(pageNo, r);
        sRect = {rScreen.x, rScreen.y, rScreen.x+rScreen.dx, rScreen.y+rScreen.dy};
        eRect = sRect;
        ...
        {            
            if (abs(pt.x - sRect.left) < iGap && abs(pt.y - sRect.top) < iGap) {
                ResizeMode = curTL;
            }
            else if (abs(pt.x - sRect.right) < iGap && abs(pt.y - sRect.top) < iGap) {
                ResizeMode = curTR;
            }
            else if (abs(pt.x - sRect.left) < iGap && abs(pt.y - sRect.bottom) < iGap) {
                ResizeMode = curBL;
            }
            else if (abs(pt.x - sRect.right) < iGap && abs(pt.y - sRect.bottom) < iGap) {
                ResizeMode = curBR;
            }
            else if (abs(pt.x - sRect.left) < iGap) {
                ResizeMode = curL;
            }
            else if (abs(pt.x - sRect.right) < iGap) {
                ResizeMode = curR;
            }
            else if (abs(pt.y - sRect.top) < iGap) {
                ResizeMode = curT;
            }
            else if (abs(pt.y - sRect.bottom) < iGap) {
                ResizeMode = curB;
            }
            else {
                ResizeMode = curNone;
            }
            // draw resize rectangle, see OnMouseMove
        ...
        if (ResizeMode == curNone) DrawMovePattern(win, pt, win->annotationBeingMovedSize);  
    }
...
static void OnMouseLeftButtonDblClk(MainWindow* win, int x, int y, WPARAM key) {
    ...
    SetTimer(win->hwndCanvas, REFRESH_TIMER_ID, REFRESH_TIMER_ID_MS, nullptr);
    return;
    // lf("Left button clicked on %d %d", x, y);
...
static void OnMouseRightButtonUp(MainWindow* win, int x, int y, WPARAM key) {
    ...
    /* return from white/black screens in presentation mode */
        ...
        OnWindowContextMenu(win, x, y);
        RECT windowRect;
        GetWindowRect(win->hwndCanvas, &windowRect);
        // restore cursor position to original right mouse button click
...
static void SetTextOrArrorCursor(DisplayModel* dm, Point pt) {
    bool stTimer;
    transferFlagStopTimer(&stTimer);
    if (dm->IsOverText(pt) || stTimer == false) {
        SetCursorCached(IDC_IBEAM);
    } else {
        SetCursorCached(IDC_ARROW);
    }
...
static void OnTimer(MainWindow* win, HWND hwnd, WPARAM timerId) {
    ...
    switch (timerId) {
        ...
        case REFRESH_TIMER_ID:
            if (GetCursor()==NULL && stTimer == false){
                SetCursorCached(IDC_IBEAM);
            }
            else
                SetCursor(NULL);
