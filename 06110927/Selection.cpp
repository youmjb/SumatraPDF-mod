...
void CopySelectionToClipboard(MainWindow* win) {
    ...
    for (int readIndex = 0; readIndex < len; readIndex++) {
        if (selText[readIndex] != '\n') {
            selText[writeIndex++] = selText[readIndex];
        }
    }
    selText[writeIndex] = '\0';
    len = strlen(selText);
    writeIndex = 0;
    for (int readIndex = 0; readIndex < len; readIndex++) {
        if (selText[readIndex] != '\r') {
            selText[writeIndex++] = selText[readIndex];
        }
    }
    selText[writeIndex] = '\0';*/
   /* for (int i = 0; selText[i] != '\0'; i++) {
        if (selText[i] == '\n' || selText[i] == '\r') {
            selText[i] = ' ';
        }
    }*/
    ...
    for (int i = 0; i < len - 1; i++) {
        if (selText[i] == '\r' && selText[i + 1] == '\n') {
            selText[i] = ' ';
            memmove(&selText[i + 1], &selText[i + 2], len - i - 1);
            len--;
            selText[len] = '\0';
        }
    }
