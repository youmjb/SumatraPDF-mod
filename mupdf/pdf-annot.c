...
pdf_dirty_annot(fz_context *ctx, pdf_annot *annot)
{
    ...
    if (ret != PDF_ANNOT_IMAGE) pdf_annot_request_resynthesis(ctx, annot);
}
...
pdf_annot_type_from_string(fz_context *ctx, const char *subtype)
{
    ...
    if (!strcmp("BBox", subtype)) return PDF_ANNOT_BBOX;
		...
		case PDF_ANNOT_FREE_TEXT:
			{
                ...
                // float CMYK[] = {0, 0.5, 0.3, 0};
				// pdf_set_annot_default_appearance(ctx, annot, "Helv", 9, 4, CMYK);
                pdf_set_annot_default_appearance(ctx, annot, "Helv", 10, nelem(red), red);
...
void pdf_set_annot_stamp_image(fz_context *ctx, pdf_annot *annot, fz_image *img, float zoom)
{
	pdf_document *doc = annot->page->doc;
	fz_buffer *buf = NULL;
	pdf_obj *res = NULL;
	...
	fz_try(ctx)
	{
        ...
        if (zoom <= 0)
            zoom = 1.0;
		rect.x1 = rect.x0 + (img->w)/zoom;
		rect.y1 = rect.y0 + (img->h)/zoom;
