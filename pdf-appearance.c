	...
	*rect = fz_empty_rect;
	...
	if (n > 0)
	{
		for (i = 0; i < n; i += 8)
		{
			...
			h = extract_quad(ctx, quad, qp, i);
			a = lerp_point(quad[LL], quad[UL], 1/24.0f);
			b = lerp_point(quad[LR], quad[UR], 1/24.0f);
...
static void pdf_write_textbox_appearance(fz_context* ctx, pdf_annot* annot, fz_buffer* buf, fz_rect* rect,
                                         pdf_obj** res) {
    fz_point quad[4];
    pdf_obj* qp;
    int i, n;
	...
	fz_text_language lang, fz_font *font, const char *fontname, float size, const char *text, const char *end)
{
	...
	while (next_text_walk(ctx, &state))
	{
        if (state.text[0] == ' ' || state.text[0] == '1' || state.text[0] == '2' || state.text[0] == '3' ||
            ...
            state.text[0] == ':' || state.text[0] == ';' || state.text[0] == '"' || state.text[0] == ',' ||
            state.text[0] == '.' || state.text[0] == '<' || state.text[0] == '>' || state.text[0] == '/' ||
                    state.text[0] == '?') state.enc = ENC_LATIN;
...
static void get_var_rect_from_text(fz_context* ctx, fz_text_language lang, fz_font* font, float size, const char* text,
                                   float* rectw, float* lineNo) {
        struct text_walk_state state;
        float x = 0;
        float xt = 0;
        float y = 0;
